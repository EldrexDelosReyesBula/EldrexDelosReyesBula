  ...
  return (
    <PageTransition>
      <section className="section-padding max-w-7xl mx-auto min-h-screen">
        ...
        >
          <span className="text-xs font-bold uppercase tracking-widest text-muted mb-4 block">Reflections</span>
          <h1 className="text-6xl md:text-8xl font-bold tracking-tighter">Quotes</h1>
        </motion.div>
        ...
        <div className="mb-16 relative max-w-xl">
          ...
          <input
            ...
            className="w-full bg-zinc-50 border border-border py-5 pl-14 pr-6 font-medium text-lg focus:outline-none focus:border-ink transition-all"
          />
        </div>

        {filteredQuotes.length === 0 ? (
          <div className="text-center py-24 border border-border bg-zinc-50">
            <p className="text-2xl font-bold text-muted">No quotes found matching "{searchTerm}"</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {filteredQuotes.map((quote, index) => (
              <motion.div
                ...
                className="border border-border p-10 flex flex-col justify-between hover:bg-zinc-50 transition-all group relative"
              >
                <div className="mb-8">
                  <Quote className="w-8 h-8 text-zinc-200 mb-6 group-hover:text-ink transition-colors" />
                  <p className="text-xl font-bold leading-relaxed tracking-tight">"{quote.text}"</p>
                </div>
                <div className="pt-8 border-t border-border">
                  <p className="text-xs font-bold uppercase tracking-widest text-muted">— {quote.author}</p>
                </div>
              </motion.div>
            ))}
          </div>
