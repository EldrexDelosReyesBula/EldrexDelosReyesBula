...
export default function Footer() {
 const { analyticsEnabled, setAnalyticsEnabled, t} = useSettings();
 ...
 <h2 className="text-3xl font-black mb-6 tracking-tight">{t('footer.quickLinks')}</h2>
 ...
 <span className="text-lg">{t(`nav.${link.toLowerCase()}`)}</span>
 <ArrowUpRight className="w-6 h-6" strokeWidth={2.5} />
 </Link>
 )
 ))}
 ...
 <span className="text-sm font-bold text-gray-400 uppercase tracking-wider">{t('settings.analytics')}</span>
 ...
 >
 <BarChart2 className="w-4 h-4" /> {t('settings.on')}
 </button>
 <button 
 ...
 <p>© {new Date().getFullYear()} Eldrex Delos Reyes Bula. {t('footer.rights')}</p>
 <p className="text-xs">{t('footer.copyright')}</p>
 </div>
 <div className="flex flex-wrap justify-center gap-4">
 <Link to="/privacy-policy" className="hover:text-[#ff0557] transition-colors hover-target">{t('footer.privacy')}</Link>
 <Link to="/terms-of-use" className="hover:text-[#ff0557] transition-colors hover-target">{t('footer.terms')}</Link>
 <Link to="/copyright" className="hover:text-[#ff0557] transition-colors hover-target">{t('footer.copyright_link')}</Link>
 <Link to="/license" className="hover:text-[#ff0557] transition-colors hover-target">{t('footer.license')}</Link>
 </div>
 </div>
 </div>
 </footer>
